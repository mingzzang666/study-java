package stringTest;

import java.util.Scanner;

public class StringTask {
   public static void main(String[] args) {
//      사용자에게 입력받은 문자열 중
//      소문자는 모두 대문자로, 대문자는 모두 소문자로 변경한다.
//      소문자 범위: 97 ~ 122
//      대문자 범위: 65 ~ 90
//      String message = "문자열: ";
//      String data = null, result = "";
//      Scanner scanner = new Scanner(System.in);
//      
//      System.out.print(message);
//      data = scanner.nextLine();
//      
//      for (int i = 0; i < data.length(); i++) {
//         char c = data.charAt(i);
//         
//         if(c >= 97 && c <= 122) {
////            소문자일 때
//            result += (char)(c - 32);
//            
//         }else if(c >= 65 && c <= 90) {
////            대문자일 때
//            result += (char)(c + 32);
//            
//         }else {
////            알파벳이 아닐 때
//            result += c;
//         }
//      }
//      
//      System.out.println(result);
      
//      사용자에게 입력받은 문자열 중
//      소문자의 총 개수를 출력한다.
//      String message = "문자열: ";
//      String data = null;
//      Scanner scanner = new Scanner(System.in);
//      
//      System.out.print(message);
//      data = scanner.nextLine();
//      int count = 0;
//      
//      for (int i = 0; i < data.length(); i++) {
//         
//         char c = data.charAt(i);
//         
//         if(c >= 97 && c <= 122) {
////            소문자일 때
////            count += 1;
//            count++;
//         }
//      }
//      
//      System.out.println(count);
      
//      아래의 주소에서 "/samsung"만 분리하여 출력한다.
//      www.naver.com/baseball/samsung
//      String url = "www.naver.com/baseball/samsung";
//      int fromIndex = url.lastIndexOf("/");
//      System.out.println(url.substring(fromIndex));
      
//      아래의 주소들을 배열에 담고, 마지막 경로가 samsung인 것만 출력한다.
//      www.naver.com/baseball/samsung
//      www.naver.com/baseball/lg
//      www.naver.com/baseball/lotte
//      www.naver.com/baseball/kt
//      www.naver.com/phone/samsung
	   
//	   		String[] url = {"www.naver.com/baseball/samsung", 
//	   										"www.naver.com/baseball/lg", 
//	   										"www.naver.com/baseball/lotte",
//	   										"www.naver.com/baseball/kt",
//	   										"www.naver.com/phone/samsung"
//	   										};
//	   		String result = "";
//	   		String target = "samsung";
//	   	
//	   		
////	   		System.out.println(url[2]);
//	   		
//	   		
//	   		for (int i = 0; i < url.length; i++) {
//	   		String[] lastUrl = url[i].split("/");
//				if(lastUrl[lastUrl.length - 1].equals(target)){
//						System.out.println(url[i]);
//				}
//			}
//	   		System.out.println(result);
	   		
//	      당신은 3칸 방이 있는 건물주이다.
//	      각 방에는 세입자가 살고있다.
//	      방마다 월세를 받아서 총 월세와 평균 월세를 구해주는 프로그램을 만들고자 한다.
	   	
//	   		Scanner sc = new Scanner(System.in);
//	   		
//	   		int[] arIncome = new int[3];
//	   		int totalRent = 0;	   	
//	   		double averageRent = 0.0;
//	   		int number = 1;
//	   		String message = 100 + number+ "호의 월세를 입력하세요.\n단위: 만 원";
//	   		
//	   		System.out.println(message);
//	   		for (int i = 0; i < arIncome.length; i++) {
//				arIncome[i] = sc.nextInt();
//				totalRent += arIncome[i];
//				if(i < 2){
//					System.out.println(100 + number + 1 + "호의 월세를 입력하세요.\n단위: 만 원");
//				number++;
//				}
//			}
//	   		averageRent = (double)totalRent / arIncome.length;
//	        averageRent= Double.parseDouble(String.format("%.2f", averageRent));
//	   		System.out.println("총 월세는 " + totalRent + "만 원 입니다.");
//	   		System.out.println("평균 월세는 " + averageRent + "만 원 입니다.");
//	   		
//	   			코드 분석
			   int[] arOriginalData = { 9, 1, 9, 2, 9 };
//			   5개의 값을 가지는 배열에 원본데이터 저장
		       boolean[] arCheck = new boolean[5];
//		       5개의 논리값 검사를 하는 배열 선언
		       int[] arData = null;
//		       출력하고자 하는 데이터 배열 선언
		       int count = 0, index = 0;
//		       카운트와 인덱스 변수 선언
		
		       for (int i = 0; i < arOriginalData.length - 1; i++) {
		          for (int j = i + 1; j < arOriginalData.length; j++) {
		             if (arOriginalData[i] == arOriginalData[j]) {
		                count++;
		                arCheck[i] = true;
		                break;
		             }
		          }
		       }
//		       반복문을 사용하여 원본 데이터의 현재 인덱스 번호의 값과  다음 인덱스 번호의 값이 동일하면 
//		       해당 인덱스가 true 값을 가지도록 논리값 배열에 저장 및 카운트를 1 올림
		       
		       arData = new int[arOriginalData.length - count];
		       for (int i = 0; i < arCheck.length; i++) {
		          if(!arCheck[i]) {
		             arData[index++] = arOriginalData[i];
		          }
		       }
		       
		       for (int i = 0; i < arData.length; i++) {
		          System.out.println(arData[i]);
		       }
	   
	   
	   
	   
	   
	   
	   
	   
	   
	   
	   
	   
	   
	   
	   
	   
	   
	   
   }
}






















